...
One honest miss worth naming rather than hiding: it found nothing on the "Tool Poisoning" challenge, because that attack is a malicious instruction embedded in a tool's docstring — a model-facing prompt-injection attack, not a code-level bug, and outside what a source-code static analyzer is built to catch.
